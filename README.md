...
The service uses Google Maps Roads API to refine manually drawn paths:
- **Validates** that paths follow real roads/bike lanes
- **Snaps** coordinates to actual road network
- **Interpolates** additional points between user clicks for accurate road following
- **Prevents** invalid paths (e.g., through water, mountains, or inaccessible areas)

### How It Works

When a user creates a manual path via `POST /paths/manual`:

1. **Original coordinates** are extracted from the segments
2. **Google Maps Roads API** is called with `snapToRoads` endpoint
3. **If successful**: Refined coordinates are used to create the path segments
4. **If it fails**: Original coordinates are used (graceful degradation)

### Getting a Google Maps API Key

1. Go to [Google Cloud Console](https://console.cloud.google.com/)
2. Create a new project or select an existing one
3. Enable the **Roads API**:
...
**Free Tier**: Google provides $200 credit per month, which equals ~40,000 requests for Roads API.
...
If the API key is not configured or the service fails, paths are stored with the original coordinates provided by the user (graceful degradation).
